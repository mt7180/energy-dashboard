...
class DataHandler:
    ...
    def read_instant_data(self, country_code: str) -> bool:
        try:
            for name, _ in self.data.items():
                self.data[name] = pd.read_pickle(
                    f"{cfd / 'tmp'}/{name}_{country_code}.pkl"
                )
        except Exception:
            return False
        return True
    ...
    def make_api_calls(self, country_code: str, forecast: bool = False) -> None:
        ...
        if DEBUG and self.read_instant_data(country_code):
            logger.debug("instant api data available and used!")
            return
        ...
        try:
            ...
            if DEBUG:
                # to: make tmp dir if not there
                for name, df in self.data.items():
                    df.to_pickle(f"{cfd / 'tmp'}/{name}_{country_code}.pkl")
        ...
        for key in self.data:
            logger.debug(f"{key=}")
            logger.debug(self.data[key].head())
    ...
    def clean_api_data(self) -> None:
        ...
        try:
            ...
            forecast_begin = (
                self.data["df_generation"].index[-1]
                if not self.data["df_generation"].empty
                else pd.Timestamp.today(tz="Europe/Brussels")
            )
            self.data["df_generation_forecast"] = self.data[
                "df_generation_forecast"
            ].loc[self.data["df_generation_forecast"].index > forecast_begin]
            self.data["df_wind_and_solar_forecast"] = self.data[
                "df_wind_and_solar_forecast"
            ].loc[self.data["df_wind_and_solar_forecast"].index > forecast_begin]

        except IndexError:
            logger.warning("IndexError occured, is dataframe empty?")
    ...
    def calculate_chart1_data(self) -> pd.DataFrame:
        if any(
            ...
            logger.debug("chart1 data calculation skipped ...")
            return pd.DataFrame()
    ...
    def calculate_chart2_data(self) -> pd.DataFrame:
        if any(
            ...
            logger.debug("chart2 data calculation skipped ...")
            return pd.DataFrame()
